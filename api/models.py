...
def init_hardware():
    ...
    hwSet1 = query.first()
    if not hwSet1:
        hw1 = Hardware(name="hwSet1", capacity=512, available=512, price=1.00)
        hw1.save(force_insert=True)
    query = Hardware.objects(name="hwSet2")
    hwSet2 = query.first()
    if not hwSet2:
        hw2 = Hardware(name="hwSet2", capacity=1024, available=1024, price=2.00)
...
def user_check_in(hw_set_name, checkin_quantity, username) -> int:
...
def user_check_out(hw_set_name, checkout_quantity, username):
    queryA = Hardware.objects(name__exact=hw_set_name)
    if len(queryA) != 1:
        return 404
    hw_set = queryA.first()
